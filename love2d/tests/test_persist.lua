...
return function(t)
  ...
  t.it("progress jsonl round-trips", function()
    ...
    t.eq(rec.quest, 1, "no quest means the first one")
  end)

  t.it("progress records the quest and its track", function()
    Store.use(scratch)
    Persist.saveProgress({ state = "play", quest = 6, track = "rust", step = 1, stage = 1, solved = false })
    local rec = Persist.loadProgress()
    t.eq(rec.quest, 6)
    t.eq(rec.track, "rust")
  end)
