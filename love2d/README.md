...
- **Quests**: seven for Go, six for Rust and Python. **Q1 BASIC** is the walk
  ...
  **Q7 THREADS** is the lunch rush, the concurrency quest: a mutex and the
  ...
  Go keeps its async and await. The Rust
  ...
  **P5 CALLBACK** and **P6 PARALLEL**, the night batch: the GIL, Process and
  ...
  `src/quests.lua` (9–19), so an old `progress.jsonl` keeps its meaning. **Q** on the title or
...
- `progress.jsonl` — `progress` records: quest (1-4 Go, 5-8 Rust, 9-11
  ...
  18 Go THREADS, 19 Python PARALLEL),
